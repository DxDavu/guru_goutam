...
export const getUserById = async (id) => {
  ...
  try {
    ...
    return {
      ...serializeData(user.toObject()),
      roles: user.roles.map((role) => serializeData({
        _id: role._id.toString(),
        role_name: role.role_name,
      })),
      departments: user.departments.map((dept) => serializeData({
        _id: dept._id.toString(),
        department_name: dept.department_name,
      })),
      branches: user.branches.map((branch) => serializeData({
        _id: branch._id.toString(),
        branch_name: branch.branch_name,
      })),
    };
    ...
    return {
      ...
      roles: user.roles?.map((role) => ({
        ...
        department: role.department
          ...
          : null,
        module_access: role.module_access?.map((module) => serializeData(module)) || [],
      })) || [],
      departments: user.departments?.map((dept) => serializeData(dept)) || [],
      branches: user.branches?.map((branch) => serializeData(branch)) || [],
    };
  } catch (error) {
    console.error("Failed to fetch user by username:", error);
    return null;
...
export const getUsers = async () => {
  ...
  return users.map((user) => serializeData(user));
};
...
export const createUser = async (currentState, userData) => {
  ...
  try {
    ...
    return {
      success: false,
      error: true,
      message: error?.errors?.[0]?.message || "Failed to create user. Please try again.",
    };
  }
};
