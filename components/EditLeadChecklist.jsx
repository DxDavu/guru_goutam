...
export default function EditLeadChecklist({ condition, onClose }) {
  const [checklistName, setChecklistName] = useState(condition.checklist_name || ''); // Added checklistName state
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch('/api/lead_checklist', { // Updated API endpoint
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to update lead checklist');
      }
